...
// Génère tous les jours pour le mois actuel
function getDaysForCurrentMonth() {
    ...
    let date = new Date(currentYear, currentMonth, 1); // 1er du mois actuel
    
    while (date.getMonth() === currentMonth && date.getFullYear() === currentYear) {
...
(async () => {
    const today = new Date();
    logger.header('PIPELINE 1 MOIS');
    logger.info(`Période: 1 dernier mois jusqu'à ${today.getMonth() + 1}/${today.getFullYear()}`);
    
    await initDB();
    await initBrowser();
    try {
        const days = getDaysForLastMonths(1);
        let totalInserted = 0;
        let daysWithRaces = 0;
